...
```bash
docker pull ghcr.io/gorecvpn/gorec-cabinet:latest
```
...
docker create --name tmp_cabinet ghcr.io/gorecvpn/gorec-cabinet:latest
...
```bash
git clone https://github.com/gorecvpn/Gorec-Cabinet.git
cd Gorec-Cabinet
cp .env.example .env
```
...
services:
  cabinet-frontend:
    image: ghcr.io/gorecvpn/gorec-cabinet:latest
...
networks:
  bot_network:
    external: true
    name: gorecbot_bot_network  # Сеть вашего бота
```
